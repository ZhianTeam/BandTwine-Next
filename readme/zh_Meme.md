...
## 功能特色
- [x] **全面兼容 Twine Twee。** 再也不用纠结「如何退出 Vim」和「为什么我的 JSON 格式又炸了」！随便用个 Twine 编辑器，导出成 Twee 文件，直接扔进你的项目里就完事儿了！再配上 [NeG Converter](/conv/QUICKSTART.md)，旧版 BandTwine 的 JSON 屎山也能一键洗干净。
- [x] **原汁原味的 Twine 体验。** 从多层变量、条件分支到随机系统和事件监听器，BandTwine Next 提供了跟 Twine 几乎一毛一样强大的功能集，完全能驱动复杂的叙事逻辑。
- [x] **优雅的 KDL 配置。** 彻底告别 `meta` 块和那一坨令人抓狂的括号 `{{{{[{[{}]}]}}}}`！`/-` 节点级注释、结构化多参数节点，写起来比 JSON 爽一百倍；编译期直接压成二进制 TLV，运行时零解析开销，手环再也不用边玩边 GC。
- [x] **专为「蒸汽机」魔改优化。** 又轻又强。支持在 Twee 里直接嵌入 JavaScript，Vela 原生 API 照常调用——亮度、传感器、震动、电池… 不是只能震的「环间跳蛋」玩具沙盒。
- [x] **少写代码，多爽一把。** 一个 BandTwine 项目，直接在小米和红米（REDMI）的小收集器和大收集器上跨设备跑起来。
